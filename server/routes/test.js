...
router.get("/studenttest", async (req, res) => {
    try {
        const student = await Students.findOne({ name: "Cheng Wai Jun" }).exec();
        const lecturer = await Lecturers.findOne({ name: "Lecturer Example" }).exec();
...
router.get("/assigntest", async (req, res) => {
    try {
        const student = await Students.findOne({ name: "Cheng Wai Jun" }).exec();
        const lecturer = await Lecturers.findOne({ name: "Lecturer Example" }).exec();
        ...
        //code to assign student to lecturer and lecturer to student:
        student.supervisor.push(lecturerID);
        lecturer.students.push(studentID);
