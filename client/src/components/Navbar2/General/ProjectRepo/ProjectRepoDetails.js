...
const ProjectRepoDetails = ({ filter, filter2, row, setConfirmDialog, confirmDelete, openForm }) => {
    ...
    return (
        <React.Fragment>
            {((user?.result?.googleId === row?.creator || user?.result?._id === row?.creator) && row.department === filter && row.status === filter2 ) && (      
            <TableRow key={row._id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                ...
                //start: below here should only be availble for admin only
                {(filter2 === 'active') && (
                ...
                //end
                //no need if admin here bcs alrdy set in ProjectRepo.js
                {(filter2 === 'pending') && (
                <TableCell className={classes.tableCell}>
                        <TableCell className={classes.tableCell}>
                        <Button
            ...
            <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
                <Collapse in={openDetail} timeout="auto" unmountOnExit>
                    <Box sx={{ margin: 12 }}>
                        ...
                        {(row.status === 'active') && (
                        <Typography variant="body2">No. of Students: {row.noOfStud}</Typography>
                        )}
                        {(row.status !== 'active') && (
                        <Typography variant="body2">Students: {row.studName}</Typography>
                        )}
                    </Box>
                </Collapse>
            </TableCell>
