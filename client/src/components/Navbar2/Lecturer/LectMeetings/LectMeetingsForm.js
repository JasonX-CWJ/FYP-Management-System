...
const LectMeetingsForm = ({ currentId, setCurrentId, setNotify, setOpenPopup }) => {
    ...
    const lecturerID = user?.result?.lecturerData?._id;
    const [lectMData, setlectMData] = useState({ title: "", projecttitle: "", studentID: "", supervisorID: lecturerID, link: "", date: "", time: "", status: "active" });
    ...
    const clearForm = () => {
        setCurrentId(0);
        setlectMData({ title: "", projecttitle: "", studentID: "", supervisorID: lecturerID, link: "", date: "", time: "", status: "active" });
    };
    ...
    const handleChange = (e) => {
        console.log(e);
        setlectMData({ ...lectMData, studentID: e.target.value });
    };
        ...
        <Container>
            <form autoComplete="off" className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
                ...
                <TextField
                    ...
                    label="Meeting Title"
                    fullWidth
                    value={lectMData.title}
                    onChange={(e) => setlectMData({ ...lectMData, title: e.target.value })}
                />
                {/* <TextField
                    name="projecttitle"
                    variant="outlined"
                    ...
                    label="Project Title"
                    fullWidth
                    value={lectMData.projecttitle}
                    onChange={(e) => setlectMData({ ...lectMData, projecttitle: e.target.value })}
                /> */}
                    ...
                    {lectPD.map((row) => (
                        <>
                            {(user?.result?.googleId === row?.creator || user?.result?._id === row?.creator) && row.status === "active" && (
                                <Select labelId="studName" id="studName" value={lectMData.studname} label="studName" onChange={handleChange}>
                                    {row.studentAssigned.map((row) => [<MenuItem value={row._id}>{row.name}</MenuItem>])}
                                </Select>
                            )}
                        </>
                    ))}
